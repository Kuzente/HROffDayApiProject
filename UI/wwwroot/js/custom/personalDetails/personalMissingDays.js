document.addEventListener('DOMContentLoaded',function () {
    ...
    if (!urlParams.has('sortName') || !urlParams.has('sortBy') || urlParams.get('sortBy') === '') {
        $('button[data-sort="sort-createdAt"]').addClass('desc');
    } else {
        if (urlParams.get('sortBy') === 'asc') {
            $('button[data-sort="sort-' + urlParams.get('sortName') + '"]').addClass('asc');
        } else {
            $('button[data-sort="sort-' + urlParams.get('sortName') + '"]').addClass('desc');
        }
    }
    $('button[data-sort]').click(function () {
        ...
        urlParams.set('sortBy', sortOrder);
        ...
        $.ajax({
            ...
            if (res.isSuccess){
                ...
                if (res.data.status === 0) { // Online
                    ...
                    $('#addModalNameSurnameInput').text(res.data.nameSurname)
                    $('#addModalBranchInput').text(res.data.branch.name)
                    $('#addModalIdentificationNumberInput').text(res.data.identificationNumber)
                }
                // Personel İşten Çıkarılmış Personel ise
                else{
                    $('a[data-bs-target="#addModal"]').remove();
    ...
    $('#addMissingDayForm').submit(function (e) {
        ...
        if (!checkRequiredFields()) {
            ...
            let formData = $("#addMissingDayForm").serializeArray();
            console.log(formData)
            $.ajax({
                type: "POST",
                url: "/personel-eksik-gun-ekle",
