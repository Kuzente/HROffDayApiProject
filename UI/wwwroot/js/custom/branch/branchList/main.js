document.addEventListener('DOMContentLoaded',function () {
    // Sayfadaki satır sayısı ve mevcut sayfa numarası
    ...
    function getODataUrl() {
        const skipCount = (currentPage - 1) * rowsPerPage;
        let odataUrl = `/query/sube-listesi?top=${rowsPerPage}&skip=${skipCount}&select=Name,Status,CreatedAt,ModifiedAt,ID,Count&orderby=Name`;
        if (searchKeyword){
            odataUrl +=  `&$filter=contains(tolower(Name), tolower('${searchKeyword}'))`;
        }
        return odataUrl;
    }
    ...
    function fetchData() {
        $.ajax({
            ...
            renderTable(res); // OData sonucunun "value" özelliğini kullanın
            if (searchKeyword){
                totalPages = Math.ceil(res.length / rowsPerPage);
            }else{
                totalPages = Math.ceil(res[0].Count / rowsPerPage);
            }
            updatePagination();
        });
    }
    function renderTable(data) {
        ...
        data.forEach(function (item) {
            ...
            row.append("<td>" + item.ModifiedAt + "</td>");
...
`);
    ...
    // İlk veriyi çek ve sayfayı oluştur
    fetchData();

    $('#searchInput').on('input', function () {
        searchKeyword = $(this).val();
        currentPage = 1; // Arama yapılıyorsa ilk sayfaya dön
        fetchData();
    });
});